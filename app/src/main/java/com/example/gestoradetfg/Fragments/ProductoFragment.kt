...
class ProductoFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        bindingProd.btnAddProducto.setOnClickListener {
            ...
            builder.setPositiveButton(R.string.crear) { view, _ ->
                ...
                if (validaCampos()) {
                    ...
                    addProducto(creaProducto(), conUsuarioActivity)

                } else {
                    Toast.makeText(
                        conUsuarioActivity,
                        R.string.err_camposVacios,
                        Toast.LENGTH_SHORT
                    ).show()
                }
            }
        ...
        bindingProd.fSpinnerTipoProducto.onItemSelectedListener=
    ...
    private fun rellenaComboProv() {
        ...
        val aaProveedores=
            ArrayAdapter<String>(conUsuarioActivity, android.R.layout.simple_spinner_dropdown_item)
    ...
    private fun creaProducto(): Producto {
        ...
        return Producto(
            "1", form_nombre.text.toString(), form_precio.text.toString().toDouble(),
            form_calidad.rating.toDouble(), tipoProducto, 0.0, getIdProv()
        )
    ...
    private fun filtraLista(tipoVenta: Int) {
        ...
        when (tipoVenta) {
            ...
            1 -> filtraPorUnidad()
            2 -> filtraPorPeso()
            else -> Toast.makeText(conUsuarioActivity, "Ha fallado el filtro", Toast.LENGTH_SHORT)
                .show()
        }
    ...
    private fun filtraVacio() {
        listaFiltradaProductos=listaProductos.filter { producto ->
            producto.nombre.lowercase()
                .contains(bindingProd.etFilterProducto.text.toString().lowercase()) &&
